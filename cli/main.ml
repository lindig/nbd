...
module Impl = struct
  ...
  let serve_common serve_export port exportname certfile ciphersuites no_tls =
...
let proxy_cmd =
  ...
  Term.(ret(pure Impl.proxy $ common_options_t $ uri_t 0 $ port $ exportname $ certfile $ ciphersuites $ no_tls)),
